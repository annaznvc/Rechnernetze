...
def b64(text):
    """Konvertiert einen ASCII-String in Base64 – so wie die Aufgabe beschreibt."""
    return base64.b64encode(text.encode('utf-8')).decode('utf-8')


def send(sock, msg):
    """Sendet eine Zeile UTF-8-kodiert mit \\r\\n – SMTP verlangt das als Zeilenende."""
    print(f">>> {msg}")
    sock.send((msg + '\r\n').encode('utf-8'))
...
recv(raw_sock)  # Server-Begrüßung: "220 ..."

# Schritt 2: EHLO senden – stellt uns vor und listet Server-Features
send(raw_sock, 'EHLO meinrechner')
recv(raw_sock)

# Schritt 3: STARTTLS anfordern
...
# Schritt 4: SSL-Socket aufbauen
# check_hostname und verify_mode deaktivieren wegen Windows-Zertifikatsproblem
...
recv(sock)  # Server fragt nach Username

send(sock, b64(USERNAME))
recv(sock)  # Server fragt nach Passwort

send(sock, b64(PASSWORD))
recv(sock)  # "235 Authentication successful"

# Schritt 6: Mail versenden
send(sock, f'MAIL FROM:<{ABSENDER_SMTP}>')
recv(sock)

send(sock, f'rcpt to:<{EMPFAENGER}>')  # Kleinbuchstaben wichtig! (siehe Aufgabe)
recv(sock)

send(sock, 'DATA')
recv(sock)  # "354 End data with <CR><LF>.<CR><LF>"

# Header und Body – Leerzeile trennt Header von Body (RFC 2822)
...
send(sock, '.')  # einzelner Punkt = Ende der Mail
